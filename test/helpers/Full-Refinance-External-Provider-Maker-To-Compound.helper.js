...
const ConnectGelatoDebtBridgeABI = require("./../../artifacts/contracts/connectors/ConnectGelatoDebtBridge.sol/ConnectGelatoDebtBridge.json")
  .abi;
...
async function makerToCompoundSetup() {
  ...
  await enableGelatoConnectorsForFromMaker(
    ...
    contracts.connectGelatoDebtBridge.address,
...
async function providerWhiteListTaskForMakerToCompound(
  ...
  const data = await contracts.debtBridgeFromMaker.getDebtBridgeFullRefinanceMakerToCompoundData(
    vaultId,
    constants.ETH,
  ...
  const debtBridgeCalculationForFullRefinance = new GelatoCoreLib.Action({
    addr: contracts.connectGelatoDebtBridge.address,
    data: await hre.run("abi-encode-withselector", {
      abi: ConnectGelatoDebtBridgeABI,
      functionname: "computeRefinanceDataAndCast",
      inputs: [contracts.debtBridgeFromMaker.address, data],
    }),
